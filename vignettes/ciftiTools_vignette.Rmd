...
cii <- read_cifti(cifti_fnames["dscalar"])
view_xifti_surface(cii, idx=1, zlim=c(1,2), widget=FALSE)
```

```{r, fig.cap=".dscalar file; second column; diverging palette", rgl=TRUE, format="png", fig.height=6, fig.width=7.5}
view_xifti_surface(cii, idx=2, zlim=c(1,5), color_mode = "diverging", widget=FALSE)
```
...
view_surf(surfL_fname, widget=FALSE)
```
...
plot(small_surf, edge_color="black", vertex_size=4, widget=FALSE)
```
...
view_xifti_surface(cii_new, title=".dtseries Mean Image", zlim=c(0,4), widget=FALSE)
```
...
plot(
  ...
  title="Smoothed CIFTI", zlim=c(1,2), widget=FALSE
)
```
