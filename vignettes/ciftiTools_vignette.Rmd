...
```{r, out.width="75%", fig.cap="First time point; sequential palette"}
knitr::include_graphics(view_xifti_surface(
  cii, zlim=c(1,2), mode='image', fname=file.path(out_dir, "xifti_0.png")
))
```

```{r, out.width="75%", fig.cap="Second time point; diverging palette"}
knitr::include_graphics(view_xifti_surface(
...
```{r, out.width="75%", fig.cap=".dlabel file; first label; qualitative palette"}
...
```{r, out.width="75%", fig.cap="Subcortical data (all ones)"}
...
```{r, out.width="50%", fig.cap="Right hemisphere surface"}
knitr::include_graphics(plot(
  make_surf(surfR_fname), hemisphere="right", mode="image", 
  fname=file.path(out_dir, "surf_1.png")
))
```
...
cii_new <- as.xifti(
  ...
  subcortVol = unmask_vol(vol, cii2$meta$subcort$mask, fill=0),
...
To visualize the cortical data of the new `xifti` object, we need to add the surface geometry:

```{r, out.width="75%", fig.cap=".dtseries mean image"}
...
Here's the subcortical data:

```{r, out.width="75%", fig.cap="Subcortical data (with random noise)"}
...
```{r}
resampled_cii_fname <- "my_new_resampled.dscalar.nii"
...
rglwidget(elementId="resampled"); rgl.close()
```
...
target_cii_fname <- file.path(out_dir, "target.dscalar.nii")
...
resample_cifti_from_template(
  original_fname=cifti_fnames$dscalar,
...
```{r, out.width="75%", fig.cap="Smoothed CIFTI"}
