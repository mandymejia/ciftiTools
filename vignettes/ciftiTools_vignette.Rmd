...
if (ciftiTools:::is_tahoe()) {
  ...
  #try(chromote::ChromoteSession$default()$close(), silent = TRUE) 
  #try(chromote::Chromote$default()$close(), silent = TRUE)
  # Sometimes the first Chromium screenshot fails.
...
# Define function for rendering images, to use only if knitting.
is_knit <- knitr::is_html_output() || knitr::is_latex_output()
knit_pic <- function(dpi=200){
  pic <- tempfile(fileext=".png")
  if (ciftiTools:::is_tahoe()) {
    suppressMessages(rgl::snapshot3d(pic, "png", delay = 3))
    # # Seems to be the same as above.
    # tmp <- tempfile(fileext = ".html")
    # htmlwidgets::saveWidget(rgl::rglwidget(), tmp, selfcontained = TRUE)
    # webshot2::webshot(tmp, "~/Downloads/temp.png", delay = 3)
  } else {
...
```{r, fig.cap="dscalar; using NA values to selectively color"}
...
```{r, fig.cap="Lessening the shadows"}
view_xifti_surface(
...
```{r, fig.cap="Exact coloring, by disabling shadows and color smoothing"}
view_xifti_surface(
...
```{r, fig.cap="Smoothed CIFTI, default palette"}
...
```{r, fig.cap="Smoothed CIFTI, RColorBrewer palette"}
# Let's demonstrate the ability to use RColorBrewer palettes!
plot(
  read_cifti(file.path(out_dir, smoothed_xii_fname)), 
  surfL=surfL_fname, surfR=surfR_fname, 
...
```{r, fig.cap="Surface with vertices drawn in blue"}
# Recall that surf was resampled to 2k
plot(surf, vertex_size=3, vertex_color="blue")
if (is_knit) { knit_pic() }
```

```{r, fig.cap="Surface with edges drawn in black"}
...
```{r fig.cap="Schaefer 400 parcellation"}
