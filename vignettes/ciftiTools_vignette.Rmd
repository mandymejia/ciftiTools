...
`apply_parc` computes a summary statistic across data for each parcel. By default, the mean is computed.

```{r}
xii <- read_cifti(ciftiTools.files()$cifti["dtseries"])
xii_parc <- load_parc("Schaefer_400")
xii_pmeans <- apply_parc(xii, xii_parc)
head(xii_pmeans)
```
To visualize the parcellated data: 

```{r, fig.cap="Mean timecourses for Schaefer 400 parcels", rgl=TRUE, format="png", fig.height=3.8, fig.width=5}
xii <- move_from_mwall(xii, NA)
xii <- newdata_xifti(xii, xii_pmeans[c(as.matrix(xii_parc)+1)])
plot(
  xii, zlim=c(1.5, 2.5), cex.title=1.3,
  colors="viridis", title="Parcellated data"
)
```
