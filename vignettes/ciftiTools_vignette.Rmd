...
These files are all in `fs_LR` coordinates, but `ciftiTools` can work with files created with other brain templates too, sich as `fsaverage`. 
...
Let's see an example using each `color_mode` option. Note how the included surfaces are used in the first plot, but if none are present as in the second and third plots, the default surfaces are automatically used for visualization. (In our case, the included surfaces _are_ the default surfaces.) We'll use widgets for each plot, but for the second plot we'll also display two `idx`. Notice how a slider is added to control which index is being visualized. Try clicking and dragging around the plots to rotate, and scrolling to zoom in and out. 

```{r, fig.cap="dtseries file; first column; sequential palette", fig.width=5}
view_xifti_surface(xii, idx=1, zlim=c(1,2), title='color_mode = "sequential"')
```

```{r, fig.cap="dtseries file; first two columns, diverging palette", fig.height=3.7, fig.width=5}
...
```{r, fig.cap="Lessening the shadows", fig.width=5}
view_xifti_surface(
...
```{r, fig.cap="Exact coloring, by disabling shadows and color smoothing", fig.width=5}
view_xifti_surface(
...
plot(
  ...
  zlim=c(.6,1.8), color_mode="diverging"
)
```
...
```{r, fig.cap="Two alternative surfaces", fig.width=5}
xii <- as.xifti(
  surfL = load_surf("left", "very inflated"),
  surfR = load_surf("right", "midthickness")
)
xii <- resample_cifti(xii, resamp_res=5000)
...
```{r, fig.cap="Surface with vertices drawn in blue", fig.width=2.5}
# Recall that surf was resampled to 2k
plot(surf, vertex_size=3, vertex_color="blue")
```

```{r, fig.cap="Surface with edges drawn in black", fig.width=2.5}
...
```{r fig.height=4.2, fig.width=5.5, fig.cap="500 evenly-sampled vertices"}
...
```{r, fig.cap="500 evenly-sampled vertices", fig.width=2.5}
...
xii <- convert_xifti(xii, "dlabel")
xii <- resample_cifti(xii, resamp_res=50000)
plot(xii)
```
...
```{r fig.cap="Schaefer 400 parcellation", fig.width=5}
...
```{r, fig.cap="Median parcel value", fig.width=5}
...
# Plot the median.
plot(psumm, idx=3, title=psumm$meta$cifti$names[3], colors="magma", zlim=c(0, 3), widget=TRUE)
```
