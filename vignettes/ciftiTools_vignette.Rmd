...
```{r, fig.cap="First time point; sequential palette", rgl=TRUE, format="png", fig.height=6, fig.width=7.5}
view_xifti_surface(cii, zlim=c(1,2))
```

```{r, fig.cap="Second time point; diverging palette", rgl=TRUE, format="png", fig.height=6, fig.width=7.5}
view_xifti_surface(cii, zlim=c(1,2))
```

```{r, fig.cap=".dlabel file; first label; palette from label metadata", rgl=TRUE, format="png", fig.height=6, fig.width=7.5}
view_xifti_surface(
  read_cifti(cifti_fnames["dlabel"], surfL=surfL_fname, surfR=surfR_fname)
)
```
...
We are working on a slider bar for viewing different columns/timepoints of the data. (It will not show up in the github htmlpreview.)
