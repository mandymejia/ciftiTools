...
rgl::par3d(windowRect = c(20, 20, 1000, 700))
rgl::text3d(
  z=200, y=0, x=0,
  cex=2,
  font=2, # Forget if this made a difference...
  color="black",
  text="Help Me Move Up"
)
```
...
```{r, webgl=FALSE, results="hide"}
x <- view_xifti_surface(
  read_cifti(cifti_fname=cii_6k["cifti"], surfL=cii_6k["surfL"], surfR=cii_6k["surfR"]), 
  idx=1:2,
  bg="#d4edf5", cex.title=1.25, zlim=c(0,2)
)
# Note: Call rglwidget() here to display inline in RStudio.
# rglwidget() does not work with htmlpreview, so we used the WebGL hook instead.
```
...
rgl::playwidget(
	rgl::rglwidget(), 
	start=0, stop=2 - 1, interval=1, 
	components="Slider", 
	rgl::subsetControl(1, subsets = x$rglIDs2)
)
rgl.close()
```
