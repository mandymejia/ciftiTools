...
```{r}
cifti_fnames <- ciftiTools::ciftiTools.files()$cifti
surfL_fname <- ciftiTools::ciftiTools.files()$surf["left"]
surfR_fname <- ciftiTools::ciftiTools.files()$surf["right"]
```
...
* `surfL` and `surfR` specify the surface geometry to plot the data on. If not provided, the "inflated" surfaces included in `ciftiTools` are used.
...
```{r}
# Reading
# ciftiTools.setOption("surf", "midthickness") # [TO DO] replace
surf <- read_surf(ciftiTools.files()$surf["left"])
surf
```
...
As shown above, the inflated surface is included as a GIFTI file and can be accessed with `ciftiTools.files()`. But two more surfaces are available through the function `load_surf`:
...
plot(xii, title = "Left very inflated surface | Right midthickness surface")
```
