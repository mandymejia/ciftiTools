...
cifti_resample_separate <- function(c_original_fnames, c_target_fnames=NULL, 
  ...
  if(length(c_original_fnames == 1) & grepl("\\.d.*.nii$", cifti_fname)){ # .d*.nii or .d*.nii where * is tseries or scalar
    ...
    c_original_fnames <- make_abs_path(c_original_fnames, read_dir)
    if(!all(file.exists(c_original_fnames))){
      stop(do.call(paste, c("This file(s) to resample does not exist:", c_original_fnames[!file.exists(c_original_fnames)])))
    }
  }
  ## c_target_fnames
  write_dir <- check_dir(write_dir)
  if(length(c_target_fnames == 1) & grepl("\\.d.*.nii$", cifti_fname)){ # .d*.nii or .d*.nii where * is tseries or scalar
    c_target_fnames <- vector("list", 0)
  } else {
  ...
  for(i in 1:length(c_original_fnames)){
    ...
    if(identical(c_target_fnames[[lab_i]], NULL)){ 
        c_target_fnames[[lab_i]] <- paste("resampled", res, c_original_fnames[[lab_i]], sep="_")
    }
  ...
  if(!is.null(surfL_original_fname)){
    ...
    if(!all(file.exists(surfL_original_fname))){
      stop(do.call(paste, c("This file(s) to resample does not exist:", surfL_original_fname[!file.exists(surfL_original_fname)])))
    }
  ...
  if(!is.null(surfR_original_fname)){
    ...
    if(!all(file.exists(surfR_original_fname))){
      stop(do.call(paste, c("This file(s) to resample does not exist:", surfR_original_fname[!file.exists(surfR_original_fname)])))
    }
...
resample_individual <- function(original_fname, target_fname, 
  ...
  read_dir=NULL, write_dir=NULL, sphere_target_dir=NULL, wb_dir=NULL){
  ...
  stopifnot(file.exists(target_fname))
...
#' @export
#'
make_helper_spheres <- function(sphereL_fname, sphereR_fname, target_res, write_dir=NULL, wb_dir=NULL){
  ...
  system(paste(wb_cmd,'-surface-create-sphere', target_res, sphereL_fname, sep=' '))
  ...
  file.exists(sphereL_fname) & file.exists(sphereR_fname)
}